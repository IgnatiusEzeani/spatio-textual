...
GAZETTEER = Path(__file__).resolve().parents[1] / "tutorials" / "sh2026" / "data" / "teaching_gazetteer.csv"
...
def test_resource_list_exposes_plural_brittleness_for_roads():
    # combined_geonouns.txt currently contains `road`, not `roads`.
    # This is a deliberate teaching failure: deterministic resources only see
    # forms represented by their patterns unless morphology/variants are added.
    ann = RuleGazetteerAnnotator(gazetteer_path=GAZETTEER, link_places=False)
