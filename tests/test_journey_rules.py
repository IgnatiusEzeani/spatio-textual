...
def test_rule_dependency_keeps_missing_destination_null(extractor):
    text = "At sunrise, Mira left Aberdeen without recording the destination."
    result = extractor.extract(text, file_id="x3")
    assert len(result["journeys"]) == 1
    journey = result["journeys"][0]
    assert journey["start_location"] == "Aberdeen"
