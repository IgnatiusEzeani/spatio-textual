...
def span_response_schema(allowed_labels: Sequence[str]) -> dict[str, Any]:
    ...
    if unknown:
        raise ValueError(f"Unsupported SH2026 labels: {unknown}")
    return {
        "type": "object",
...
def build_span_prompt(source_text: str, *, allowed_labels: Sequence[str] = TOPONYM_LABELS) -> str:
    ...
    scope = (
        ...
        else "Use the broader SH2026 spatial ontology below and annotate the smallest literal source span supporting each label."
    )
    return (
...
class LLMSpanExtractor:
    ...
    def extract(self, source_text: str, *, file_id: str) -> dict[str, Any]:
        ...
        return {
            "schema_version": "sh2026-llm-spans-v1",
