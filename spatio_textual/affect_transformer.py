...
# surprise. SH2026 v1 intentionally excludes disgust and does not treat neutral
...
REPRESENTABLE_SH2026_EMOTIONS = ("anger", "fear", "joy", "sadness", "surprise")
UNSUPPORTED_SH2026_EMOTIONS = ("anxiety", "despair", "gratitude")
...
def _emotion_labels(dist: dict[str, float]) -> list[str]:
    labels = [
        label
        for label in REPRESENTABLE_SH2026_EMOTIONS
        if dist.get(label, 0.0) >= EMOTION_THRESHOLD
    ]
    if labels:
        return sorted(labels)
    supported_scores = {label: dist.get(label, 0.0) for label in REPRESENTABLE_SH2026_EMOTIONS}
