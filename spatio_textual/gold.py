...
def load_gold_jsonl(path: str | Path) -> list[dict[str, Any]]:
    """Load one SH2026 gold/reference record per JSONL line.
...
def validate_gold_record(record: dict[str, Any]) -> list[str]:
    ...
    if record.get("schema_version") != "sh2026-gold-0.1":
...
def assert_valid_gold(records: Sequence[dict[str, Any]]) -> None:
    ...
    if errors:
        ...
        raise ValueError(f"Invalid SH2026 gold/reference data:\n{preview}")
...
def find_span(
    ...
    *,
    ...
    if label not in SPAN_LABELS:
        raise ValueError(f"Unsupported SH2026 span label: {label}")
    return {
        "span_id": span_id,
        "layer": layer,
        "label": label,
